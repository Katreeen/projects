		...
		if (!!modef && !!modef_num)
		{
			...
			else
			{
				...
				$.get(
					...
					function (data) {
						$('.common__list').html($(data).find('.common__list').html());
						var top = $('.common__wrap').offset().top;
						$('html, body').animate({
							scrollTop: top
						}, 700);
					}
				...
				element.classList.add('filter-active-item');
				element.setAttribute("data-id", this.curFilterinput.id);
				element.textContent = this.curFilterinput.nextElementSibling.textContent;
				let rem = 0;
				filterActive.classList.add('active');
				if (filterActive.children.length > 0) {

					for (const child of filterActive.children) {
						if (child.getAttribute("data-id") == this.curFilterinput.id) {
							child.remove();
							rem = 1;
						}
					}
					if (rem != 1) {
						filterActive.appendChild(element);
					}
