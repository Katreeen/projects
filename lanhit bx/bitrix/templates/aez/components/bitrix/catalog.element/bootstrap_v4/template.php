...
?>
<div class="product bx-catalog-element<?= $themeClass ?>" id="<?= $itemIds['ID'] ?>" itemscope itemtype="http://schema.org/Product">
	<?php
	if ($arParams['DISPLAY_NAME'] === 'Y') {
	?>
		<div class="product__title product__title--mobile">
			<h1 class="title"><?= $name ?></h1>
			<div class="product__subtitle"><?= $arResult["PROPERTIES"]["SUBTITLE"]["VALUE"] ?></div>
		</div>
	<?php
	}
	?>
	...
	<div class="product__head">
		<!-- <div id="demo-container"></div> -->
			<div class="row">
				<div class="col-lg-4">
					<div class="product__part product__part--left">
						<? if ($haveOffers) {
							foreach ($arResult['OFFERS'] as $keyOffer => $offer) {
								if (!isset($offer['MORE_PHOTO_COUNT']) || $offer['MORE_PHOTO_COUNT'] <= 0)
									continue;
								$strVisible = $arResult['OFFERS_SELECTED'] == $keyOffer ? '' : 'none'; ?>
								<div class="product__sliders" id="<?= $itemIds['SLIDER_CONT_OF_ID'] . $offer['ID'] ?>" style="display: <?= $strVisible ?>;">
									<div class="sliders">
	
										<div class="slider js-product-slider 000">
											<div class="swiper-wrapper">
												...
												foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo) {
													...
													<div class="swiper-slide slider__slide 11">
														<img class="js-img-zoom" src="<?=$photo['SRC']?>" alt="<?= $name ?>">
													</div>

													<?}else{
														$max_photo = CFile::ResizeImageGet($photo['ID'], array('width' => 1000, 'height' => 1000), BX_RESIZE_IMAGE_PROPORTIONAL, true);
														$min_photo = CFile::ResizeImageGet($photo['ID'], array('width' => 337, 'height' => 270), BX_RESIZE_IMAGE_PROPORTIONAL, true);?>

													<div class="swiper-slide slider__slide 11">
														<a href="<?=$max_photo['src']?>" data-fancybox>
															<img class="js-img-zoom" src="<?=$min_photo['src']?>" alt="<?= $name ?>">
														</a>
													</div>
									...
									<? if(count($offer['MORE_PHOTO']) > 1){?>
										<div class="slider-thumbs">
											<div class="slider-thumbs__container js-thumbs-slider">
												<div class="swiper-wrapper">
													<?
													foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo) {

														$min_photo = CFile::ResizeImageGet($photo['ID'], array('width' => 88, 'height' => 68), BX_RESIZE_IMAGE_EXACT, true);
														
													?>
														<div class="swiper-slide slider-thumbs__slide">
															<img class="js-init-img" src="<?=$min_photo?>" alt="<?= $name ?>" itemprop="image" data-zoom-image="<?= $min_photo['src'] ?>">
														</div>
													<?
													}
													?>
													<? if ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] != '') {
														...
														foreach ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $photo) {
															$min_photo = CFile::ResizeImageGet($photo, array('width' => 88, 'height' => 68), BX_RESIZE_IMAGE_EXACT, true);
															
													?>
															<div class="swiper-slide slider-thumbs__slide">
																<img class="js-init-img" src="<?=$min_photo['src'] ?>" alt="<?= $name ?>" itemprop="image" data-zoom-image="<?= $big_photo['src'] ?>">
															</div>
													<?
														}
													} ?>
							...
							<div class="product__sliders">
								<div class="sliders">
									<div class="slider js-product-slider 223">
										<div class="swiper-wrapper">
											...
											?>
													<div class="swiper-slide slider__slide">
														<a href="<?=$max_photo['src']?>" data-fancybox>
															<img class="js-img-zoom" src="<?=$min_photo['src'] ?>" alt="<?= $name ?> 2">
														</a>
													</div>
											<?
												}
											...
											foreach ($offer['MORE_PHOTO'] as $keyPhoto => $photo) {
												...
												$min_photo = CFile::ResizeImageGet($photo['ID'], array('width' => 337, 'height' => 270), BX_RESIZE_IMAGE_PROPORTIONAL, true);
											?>
												<div class="swiper-slide slider__slide product-item-detail-slider-controls-image<?= ($keyPhoto == 0 ? ' active' : '') ?>" data-entity="slider-control" data-value="<?= $offer['ID'] . '_' . $photo['ID'] ?>">
													<a href="<?=$max_photo['src']?>" data-fancybox>
														<img class="js-img-zoom" src="<?=$min_photo['src'] ?>" alt="<?= $name ?>">
													</a>
												</div>
											...
											<?
												if($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] != ''){
													foreach ($arResult['PROPERTIES']['MORE_PHOTO']['VALUE'] as $photo)
													{	
														$max_photo = CFile::ResizeImageGet($photo, array('width'=>1000, 'height'=>1000), BX_RESIZE_IMAGE_PROPORTIONAL, true);
														$min_photo = CFile::ResizeImageGet($photo, array('width'=>337, 'height'=>270), BX_RESIZE_IMAGE_PROPORTIONAL, true);
														?>
														<div class="swiper-slide slider__slide">
															<a href="<?=$max_photo['src']?>" data-fancybox>
																<img class="js-img-zoom" src="<?=$min_photo['src'] ?>" alt="<?= $name ?>">
											...
											<div class="swiper-wrapper">
												...
												?>
														<div class="swiper-slide slider-thumbs__slide">
															...
															<img class="js-init-img" src="<?=$min_photo['src']?>" alt="<?= $name ?>" data-zoom-image="<?= $min_photo['src'] ?>">
						...
						<div class="product__docs">
							<? if (($arResult["PROPERTIES"]["PASPORT"]["VALUE"] != '') && ($USER->IsAuthorized())) {
								$pasportFile = CFile::GetFileArray($arResult["PROPERTIES"]["PASPORT"]["VALUE"]);
								$pasportLink = $pasportFile["SRC"];
								$pasportSize = CFile::FormatSize($pasportFile["FILE_SIZE"]);
								$pasportType = explode('/', $pasportFile["CONTENT_TYPE"]);
								$pasportType = strtoupper($pasportType[1]);
							?>

								<div class="product__doc">
									<a class="card card--small" href="<?= $pasportLink ?>" download="download">
										<div class="card__icon">
											<img src="<?=SITE_TEMPLATE_PATH?>/images/coa-detail.svg" alt="">
										</div>
										<!-- <div class="card__name">Паспорт безопасности</div> -->
										<div class="card__desc"><?= $pasportType ?>, <?= $pasportSize ?></div>
									</a>
								</div>
							<? } ?>
							<? if ($arResult["PROPERTIES"]["SPEC"]["VALUE"] != '') {
								$specFile = CFile::GetFileArray($arResult["PROPERTIES"]["SPEC"]["VALUE"]);
								$specLink = $specFile["SRC"];
								$specSize = CFile::FormatSize($specFile["FILE_SIZE"]);
								$specType = explode('/', $specFile["CONTENT_TYPE"]);
								$specType = strtoupper($specType[1]);
							?>
								<div class="product__doc">
									<a class="card card--small" href="<?= $specLink ?>" download="download">
										<div class="card__icon">
											<img src="<?=SITE_TEMPLATE_PATH?>/images/sds_detail.svg" alt="">
										</div>
										<!-- <div class="card__name">Спецификация</div> -->
										<div class="card__desc"><?= $specType ?>, <?= $specSize ?></div>
									</a>
								</div>
							<? } ?>
						</div>
					...
					<div class="product__part">
						...
						<div class="product__details">
							...
							<? foreach ($arResult["DISPLAY_PROPERTIES"] as $property) {
								$class = '';
								if ($property['NAME'] == "EINECS") {
									$class = 'product__detail-m';
								}
								if ($property['NAME'] == "ID") {
									$class = 'product__detail-s';
								}
								if ($property['ID'] == "118" ) continue; ?>
								<div class="product__detail <?= $class ?>">
									<div class="product__name"><?= $property['NAME'] ?>:</div>
									<a class="product__text text-attention" href="#">
						...
						<div class="product__details product__details--width">
							<div class="product__detail">
								<div class="product__name"><?= $arResult["DISPLAY_PROPERTIES"]["OTHERS_NAMES"]["NAME"] ?>:</div>
								<div class="product__text product__text--bold">
									<?= $arResult["DISPLAY_PROPERTIES"]["OTHERS_NAMES"]["~VALUE"] ?>
								</div>
							</div>
						</div>
						...
						case 'sku':
							if ($showOffersBlock) {?>
								<div class="product__table">
									<div class="table-detail" id="<?= $itemIds['TREE_ID'] ?>">
										...
										?>
											...
											<div data-entity="sku-line-block" class="table-detail__body">
																...
																?>
																	...
																	<?//if ($value['ID'] != 0){?>
																			...
																			<li class="table-detail__line product-item-scu-item-text-container" title="<?=$value['NAME']?>"
																				...
																				<div class="table-detail__column table-detail__column--s">
																					<div class="table-detail__text"><?= $arResult['PROPERTIES']['PRODUCT_ID']['~VALUE']?></div>
																				</div>
																				<div class="product-item-scu-item-text-block table-detail__column table-detail__column--m">
																					<div class="product-item-scu-item-text table-detail__text"><?= $value['NAME'] ?> g</div>
																				</div>
																				<div class="table-detail__column table-detail__column--l">
																	
																					<a class="table-detail__add js-link-add" href="#">Add to Request</a>
				...
				?>
						...
						case 'price':
							?>
								...
								<div class=" table-detail__add product-item-detail-price-current mb-1" id="<?= $itemIds['PRICE_ID'] ?>" style="display:none"><?= $price['PRINT_RATIO_PRICE'] ?></div>
						...
						case 'priceRanges':
							...
							?>
								<div class="mb-3" <?= $showRanges ? '' : 'style="display: none;"' ?> data-entity="price-ranges-block">
									...
									?>
										<div class="product-item-detail-info-container-title text-center" style="display: none">
						...
						case 'quantity':
							if ($arParams['USE_PRODUCT_QUANTITY']) {
								?>
								<div class="table-detail__number" <?= (!$actualItem['CAN_BUY'] ? ' style="display: none;"' : '') ?> data-entity="quantity-block">
									<?php
									...
									<div class="product-item-amount">
										<div class="product-item-amount-field-container input-number">
											<button class="product-item-amount-field-btn-minus no-select input-number__minus" type="button" id="<?=$itemIds['QUANTITY_DOWN_ID']?>">
												
											</button>

											<div class="product-item-amount-field-block">
												<input class="product-item-amount-field input-number__input" id="<?=$itemIds['QUANTITY_ID']?>" type="number" value="<?=$price['MIN_QUANTITY']?>">
											...
											<button class="product-item-amount-field-btn-plus no-select input-number__plus" type="button" id="<?=$itemIds['QUANTITY_UP_ID']?>">
							...
							<div data-entity="main-button-container" class="mb-3">
								<div id="<?= $itemIds['BASKET_ACTIONS_ID'] ?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none') ?>;">
									...
									?>
											...
											<a class="product-item-detail-buy-button table-detail__add" id="<?=$itemIds['ADD_BASKET_LINK']?>" href="javascript:void(0);">
									...
									?>
											...
											<a class="product-item-detail-buy-button table-detail__add" id="<?=$itemIds['BUY_LINK']?>" href="javascript:void(0);">
...
<div class="product__docs--mobile">
	...
	<? if ($arResult["PROPERTIES"]["SPEC"]["VALUE"] != '') { ?>
		<div class="product__doc">
			<a class="card card--small" href="#" download="download">
				<div class="card__icon">
					<img src="<?=SITE_TEMPLATE_PATH?>/images/sds_detail.svg" alt="">
				</div>
				<!-- <div class="card__name">Спецификация</div> -->
...
<div class="product__body">
	<div class="card-details" itemprop="description">
		<h2 class="title title--second"><?= $arParams['MESS_DESCRIPTION_TAB'] ?></h2>
		<div class="card-details__links">
			
			<?/* if ($arResult["PROPERTIES"]["TAGS"]["~VALUE"] != '') {
				foreach ($arResult["PROPERTIES"]["TAGS"]["~VALUE"] as $key => $tag) { 
					$href = $arResult["SECTION"]["SECTION_PAGE_URL"].'filter/tags-is-'.$arResult["PROPERTIES"]["TAGS"]["VALUE_XML_ID"][$key].'/apply/';
					?>
					<a class="card-details__link decor decor--l-blue" href="<?= $href;?>"><?= $tag ?></a>
			<? }
			}	*/?>

<? if ($arResult["PROPERTIES"]["YADRO"]["~VALUE"] != '') {
				foreach ($arResult["PROPERTIES"]["YADRO"]["~VALUE"] as $key => $tag) { 
					$href = $arResult["SECTION"]["SECTION_PAGE_URL"].'filter/yadro-is-'.$arResult["PROPERTIES"]["YADRO"]["VALUE_XML_ID"][$key].'/apply/';
					?>
					<a class="card-details__link decor decor--l-blue" href="<?= $href;?>"><?= $tag; ?></a>
			<? }
			}	?>

			<? if ($arResult["PROPERTIES"]["INOE"]["~VALUE"] != '') {
					$href = $arResult["SECTION"]["SECTION_PAGE_URL"].'filter/inoe-is-'.mb_strtolower($arResult["PROPERTIES"]["INOE"]["VALUE"]).'/apply/';
					?>
					<a class="card-details__link decor decor--l-blue" href="<?= $href;?>"><?= $arResult["PROPERTIES"]["INOE"]["~VALUE"] ?></a>
			<? 
			}?>

<? if ($arResult["PROPERTIES"]["SOEDINENIYA"]["~VALUE"] != '') {
					$href = $arResult["SECTION"]["SECTION_PAGE_URL"].'filter/soedineniya-is-'.$arResult["PROPERTIES"]["SOEDINENIYA"]["VALUE_XML_ID"].'/apply/';
					?>
					<a class="card-details__link decor decor--l-blue" href="<?= $href;?>"><?= $arResult["PROPERTIES"]["SOEDINENIYA"]["~VALUE"] ?></a>
			<? 
			}	?>
		</div>
		...
		<? if ($arResult["PROPERTIES"]["USEING"]["~VALUE"] != '') { ?>
			<div class="card-details__title">Application</div>
			<?= $arResult["PROPERTIES"]["USEING"]["~VALUE"]["TEXT"] ?>
		<? } ?>
		<? if ($arResult["PROPERTIES"]["PACKING"]["~VALUE"] != '') { ?>
			<div class="card-details__title">Packing</div>
			<?= $arResult["PROPERTIES"]["PACKING"]["~VALUE"]["TEXT"] ?>
		<? } ?>
	</div>
	<div class="card-details">
		<h2 class="title title--second"><?= $arParams['MESS_PROPERTIES_TAB'] ?></h2>
		<?
		$arProps1 = [];
		$arProps2 = [];

		foreach ($arResult["PROPERTIES"] as $props) {
			if ($props["ID"] >= 120 && $props["ID"] <= 126) {
				array_push($arProps1, $props);
			} else if ($props["ID"] >= 127 && $props["ID"] <= 131) {
				array_push($arProps2, $props);
			}
		} ?>

		<div class="card-details__properties">
			<? foreach ($arProps1 as $item) { ?>
				<div class="card-details__property">
					<div class="card-details__name"><?= $item["NAME"] ?>:</div>
					<?if($item["~VALUE"] != ''){?>
						<div class="card-details__text"><?= $item["~VALUE"] ?></div>
					<?}else{?>
						<div class="card-details__text"> - </div>
					<?}?>
	...
	<div class="card-details">
		<h2 class="title title--second">Information about hazards and transportation</h2>
		<div class="card-details__list">
			<? foreach ($arProps2 as $item) { ?>
				<? if ($item["ID"] == 130) { ?>
					<div class="card-details__item">
						<div class="card-details__name"><?= $item["NAME"] ?>:</div>
						<div class="card-details__text">
								<?if($item["VALUE"] != ''){?>
									<div class="decor decor--red"><?=$item["VALUE"] ?></div>
								<?}else{?>
									<div class="decor"> - </div>
								<?}?>
						</div>
			...
			<? } ?>
			<div class="card-details__item">
				<div class="card-details__name">Brief hazard characteristics:</div>
				<div class="card-details__text card-details__decor">
					<div class="decor"><?= $arResult["PROPERTIES"]["HAZARD_CHAR_FORMULA"]["~VALUE"] ?></div>
				</div>
			</div>
			<div class="card-details__item">
				<? if($arResult["PROPERTIES"]["HAZARD_CHAR"]["~VALUE"] != ''){?>
					<div class="card-details__text -gray">
					<?= $arResult["PROPERTIES"]["HAZARD_CHAR"]["~VALUE"]["TEXT"] ?>
					</div>
				<?}?>
			</div>
				<div class="card-details__item">
					<div class="card-details__name">Precautions:</div>
					<div class="card-details__text card-details__decor">
						<div class="decor"><?= $arResult["PROPERTIES"]["SAFETY_FORMULA"]["~VALUE"] ?></div>
					</div>
				</div>
				<div class="card-details__item">
				<? if($arResult["PROPERTIES"]["SAFETY"]["~VALUE"] != ''){?>
					<div class="card-details__text -gray">
						<?= $arResult["PROPERTIES"]["SAFETY"]["~VALUE"]["TEXT"] ?>
					</div>
				<?}?>
				</div>


			<div class="card-details__item -mb-0">
			<div class="card-details__name">Danger pictograms:
				<?if($arResult["PROPERTIES"]["PICTOGRAPH"]["VALUE"] != ''){?>
				<div class="pic-info">Hover over the icon to find out more</div>
				<?}?>
			</div>

			<div class="card-details__text card-details__pic">
				<?
				foreach ($arResult["PROPERTIES"]["PICTOGRAPH"]["VALUE"] as $item) {
				} ?>
				<? if (CModule::IncludeModule('highloadblock')) {
					...
					$result = $hlDataClass::getList(array(
						"select" => array("ID", "UF_NAME", "UF_FILE", "UF_XML_ID", "UF_DESCRIPTION"), // Поля для выборки
						"order" => array("UF_SORT" => "ASC"),
						"filter" => array("UF_XML_ID" => $arResult["PROPERTIES"]["PICTOGRAPH"]["~VALUE"]),
					));
					$resultAll = $hlDataClass::getList(array(
						"select" => array("ID", "UF_NAME", "UF_FILE", "UF_XML_ID", "UF_DESCRIPTION"), // Поля для выборки
						"order" => array("UF_SORT" => "ASC"),
						"filter" => array(),
					));
					...
					while ($res = $result->fetch()) {
						$arrTitle[] = $res["UF_NAME"];
						$arrDescr[] = $res["UF_DESCRIPTION"];
						$arrPic[] = $res["UF_FILE"];
					}
				} ?>
				<div class="card-details__pictograms">
					<div class="pictograms-list">
						<div class="pictograms-list__wrap">
							<? foreach ($arrPic as $key => $pic) {
								$pic70 = CFile::ResizeImageGet($pic, array('width' => 70, 'height' => 70), BX_RESIZE_IMAGE_PROPORTIONAL, true);
								$pic = CFile::ResizeImageGet($pic, array('width' => 50, 'height' => 50), BX_RESIZE_IMAGE_PROPORTIONAL, true);
								?>

								<div class="pictograms-list__item">
									<img src="<?= $pic['src'] ?>">
									<div class="pictogram-info">
										
											<div class="pictogram-info-row">
												<div class="pictogram-info-img">
													<img src="<?= $pic70['src'] ?>" alt="">
												</div>
												<div class="pictogram-info-text">
													<div class="pictogram-info-ttl"><?=$arrTitle[$key]?></div>
													<div class="pictogram-info-txt"><?=$arrDescr[$key]?></div>
												</div>
											</div>
	...
	<div class="card-details">
	<h2 class="title title--second">Information</h2>
	<p>All prices do not include VAT, shipping costs, and possibly applicable fees (Section 7 of the GTC). Prices may deviate for certain countries, product lines, and hazardous materials classes.</p>
